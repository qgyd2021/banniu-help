...
# -*- coding: utf-8 -*-
"""
根据媒资审核写入的标记计算「视频/媒体流」审核分并流转。

说明：优先读取 ``post_review_video_review.image_marks``；为兼容历史流程，
若不存在则回退读取 ``post_review_image_review.image_marks``（键为展示用 URL，含
``/api/media-stream`` 等）。

规则与 ``PostReviewGetImageScoreTask`` 一致：
- 符合 -> 100
- 空字符串 -> 90
- 不符合 -> 0
取分数最高的 ``top_k`` 条标记求平均，作为最终分（0-100，四舍五入取整）。
"""
import asyncio
...
@BaseTask.register("post_review_get_video_score")
class PostReviewGetVideoScoreTask(BaseTask, TaskJsonUtils):
    """
    根据视频审核标记计算视频（媒体流）审核分并流转。
    """
    ...
    def _calculate_video_score(self, payload: dict) -> Dict[str, object]:
        review_obj = payload.get("post_review_video_review")
        if not isinstance(review_obj, dict):
            review_obj = payload.get("post_review_image_review")
        image_marks: Dict[str, object] = {}
        ...
        for media_url, label_raw in image_marks.items():
        ...
        if not rows:
            return {
                "score": 0,
                "desc": "未找到 image_marks，无法计算视频审核分。",
                "media_count": 0,
        ...
        desc = (
            f"共 {len(rows)} 条媒体标记：符合 {label_count['符合']} 条，不符合 {label_count['不符合']} 条，空 {label_count['空']} 条。"
            f"按规则映射分值（符合=100，空=90，不符合=0），取最高 {len(top_scores)} 条参与计算，"
    ...
    async def do_task(self):
        ...
        for platform, source_dir, target_dir in self.platform_to_dir:
            ...
            for src in files:
                ...
                score_result = self._calculate_video_score(payload)
                payload["post_review_get_video_score"] = score_result
                payload["post_review_get_video_score_status"] = "done"
                payload["post_review_get_video_score_reason"] = "top_k_stream_marks_average"
                ...
                moved += 1
                logger.info(
                    f"{self.flag}视频评分完成并流转: {src.name} -> {final.as_posix()}, "
...
def main():
    ...
    task = PostReviewGetVideoScoreTask(
        check_interval=60,
        platform_to_dirs=[
            (
                "bilibili",
                "temp/banniu_37728/step_8_post_review_image_review/bilibili",
