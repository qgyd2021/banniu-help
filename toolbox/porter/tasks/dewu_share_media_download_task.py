...
from toolbox.dewu.media.share_media_download import ShareMediaDownload


@BaseTask.register("dewu_share_media_download")
class DewuShareMediaDownloadTask(BaseTask, TaskJsonUtils):
    """
    将 source_dir 下的得物 task 流转到 output_dir，并补充贴子元信息（不下载图片/视频）。
    """

    _DEWU_URL_PATTERNS = [
        r"https?://dw4\.co/[A-Za-z0-9\-._~:/?#\[\]@!$&'()*+,;=%]+",
        r"https?://m\.dewu\.com/[A-Za-z0-9\-._~:/?#\[\]@!$&'()*+,;=%]+",
        r"https?://(?:www\.)?dewu\.com/[A-Za-z0-9\-._~:/?#\[\]@!$&'()*+,;=%]+",
    ]
    ...
    def _extract_urls_from_value(value: object) -> List[str]:
        ...
        if isinstance(value, str):
            ...
            for item in value:
                urls.extend(DewuShareMediaDownloadTask._extract_urls_from_value(item))
        elif isinstance(value, dict):
            for _, v in value.items():
                urls.extend(DewuShareMediaDownloadTask._extract_urls_from_value(v))
        return list(dict.fromkeys(urls))

    @classmethod
    def _extract_dewu_share_urls(cls, payload: dict) -> List[str]:
        ...
        for url in urls:
            low = url.lower()
            for pat in cls._DEWU_URL_PATTERNS:
    ...
    async def _append_post_meta_to_task_file(self, task_file: Path, meta_rows: List[dict]) -> str:
        ...
        if not isinstance(payload, dict):
            payload = {}
        payload["dewu_post_meta_list"] = meta_rows
        payload["dewu_post_meta_updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    async def do_task(self):
        ...
        for task_file in files:
            try:
                ...
                continue
            urls = self._extract_dewu_share_urls(payload)
            if not urls:
                continue
            ...
            for share_url in urls:
                try:
                    ...
                    logger.error(f"{self.flag}拉取得物元信息失败: url={share_url}, err={e}")
                    continue
...
def main():
    ...
    task = DewuShareMediaDownloadTask(
        check_interval=60,
        source_dir="data/banniu_task_download/routed/dewu",
        output_dir="data/dewu_share_media_download/tasks",
    )
