...
@BaseTask.register("xiaohongshu_share_media_download")
class XiaoHongShuShareMediaDownloadTask(BaseTask, TaskJsonUtils):
    """
    将 source_dir 下的小红书 task 流转到 output_dir，并补充贴子元信息（不下载图片/视频）。
    """
    ...
    def _extract_urls_from_value(value: object) -> List[str]:
        ...
        if isinstance(value, str):
            ...
            for item in value:
                urls.extend(XiaoHongShuShareMediaDownloadTask._extract_urls_from_value(item))
        elif isinstance(value, dict):
            for _, v in value.items():
                urls.extend(XiaoHongShuShareMediaDownloadTask._extract_urls_from_value(v))
        return list(dict.fromkeys(urls))

    @classmethod
    def _extract_xhs_share_urls(cls, payload: dict) -> List[str]:
        ...
        for url in urls:
            low = url.lower()
            for pat in cls._XHS_URL_PATTERNS:
    ...
    async def _append_post_meta_to_task_file(self, task_file: Path, meta_rows: List[dict]) -> str:
        ...
        if not isinstance(payload, dict):
            payload = {}
        payload["xiaohongshu_post_meta_list"] = meta_rows
        payload["xiaohongshu_post_meta_updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    async def do_task(self):
        ...
        for task_file in files:
            try:
                ...
                continue
            urls = self._extract_xhs_share_urls(payload)
            if not urls:
                continue
            ...
            for share_url in urls:
                try:
                    ...
                    logger.error(f"{self.flag}拉取小红书元信息失败: url={share_url}, err={e}")
                    continue
...
def main():
    ...
    task = XiaoHongShuShareMediaDownloadTask(
        check_interval=60,
        source_dir="data/banniu_task_download/routed/xiaohongshu",
        output_dir="data/xiaohongshu_share_media_download/tasks",
    )
